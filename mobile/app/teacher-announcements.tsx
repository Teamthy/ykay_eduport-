...
export default function TeacherAnnouncements() {
  ...
  async function load() { try { setData(await teacherApi.announcements()); } catch { toast("We couldn’t refresh announcements right now.", "error"); } finally { setRefreshing(false); } }
  useEffect(() => { load(); }, []);
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background.primary }} contentContainerStyle={{ padding: spacing.lg, paddingTop: 56 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} tintColor={colors.brand.greenLight} />}>
      ...
      {items.length > 0 ? (
        items.map((n: any) => (
          <ListItem key={n.id} leftIcon={iconFor(n.kind)} accentColor={toneFor(n.kind)} title={n.title} subtitle={n.body ? `${timeAgo(n.at)} · ${n.body}` : timeAgo(n.at)} unread={!n.read} right={!n.read ? <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: colors.brand.greenLight }} /> : undefined} />
        ))
      ) : (
        <EmptyState icon={<Bell size={48} color={colors.border.strong} />} title="No announcements" />
      )}
    </ScrollView>
  );
}
