...
export default function TeacherAnnouncements() {
  ...
  async function load() {
    try {
      setData(await teacherApi.announcements());
    } catch {
    } finally {
      setRefreshing(false);
    }
  ...
  return (
    ...
    >
      ...
      {items.length > 0 ? (
        <View style={{ gap: 10 }}>
          {items.map((n: any) => (
            <View key={n.id} style={{ backgroundColor: n.read ? "#051650" : "#0A2472", borderRadius: 14, padding: 16, borderLeftWidth: 3, borderLeftColor: n.kind === "ALERT" ? "#ff4444" : "#2840E8" }}>
        ...
        <View style={{ alignItems: "center", paddingVertical: 60 }}>
          ...
          <Text style={{ color: "#ffffff40", marginTop: 12 }}>No announcements</Text>
        </View>
      )}
    </ScrollView>
  );
