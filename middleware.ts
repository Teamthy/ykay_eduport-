...
const protectedPrefixes = ["/admin", "/admin-admissions", "/teacher", "/student", "/parent"];
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    const allowed = (adminPath && ["ADMIN", "DIRECTOR", "COORDINATOR", "BURSAR"].includes(role)) || (teacherPath && ["TEACHER", "HOD", "ADMIN", "DIRECTOR"].includes(role)) || (parentPath && role === "PARENT") || (studentPath && role === "STUDENT");
...
export const config = { matcher: ["/admin/:path*", "/admin-admissions/:path*", "/teacher/:path*", "/student/:path*", "/parent/:path*"] };
