        ...
        <section className="pt-24 pb-10 bg-brand-navy px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-white/60 text-sm">
              Browse and manage the school-wide question bank.
            </p>
          </div>
        </section>
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)]">
                  ...
                  <div className="font-display text-2xl text-[var(--text-primary)]">
                    {new Set(ALL_QUESTIONS.map((q) => q.subject)).size}
                  </div>
                  <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">
                    Subjects
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)]">
                  ...
                  <div className="font-display text-2xl text-[var(--text-primary)]">
                    {ALL_QUESTIONS.filter((q) => q.waecYear).length}
                  </div>
                  <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">
                    WAEC Questions
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)]">
                  ...
                  <div className="font-display text-2xl text-[var(--text-primary)]">
                    {ALL_QUESTIONS.filter((q) => q.type === "essay").length}
                  </div>
                  <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">
                    Essay Questions
                  </div>
              ...
              <div className="space-y-3">
                {filtered.map((q, i) => (
                  <div
                    key={q.id}
                    className="p-5 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)] shadow-[var(--card-shadow)] hover:border-brand-green/30 transition-all"
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <span className="text-[9px] px-2 py-0.5 rounded bg-brand-green/10 text-brand-green font-bold uppercase">
                            {q.type.replace("_", " ")}
                          </span>
                          <span className="text-[9px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-500 font-bold">
                            {q.subject}
                          </span>
                          <span className="text-[9px] px-2 py-0.5 rounded bg-[var(--surface-disabled)] text-[var(--text-muted)] font-bold">
                            {q.topic}
                          </span>
                          <span
                            className={`text-[9px] px-2 py-0.5 rounded font-bold ${q.difficulty === "easy" ? "bg-brand-green/10 text-brand-green" : q.difficulty === "medium" ? "bg-brand-orange/10 text-brand-orange" : "bg-red-500/10 text-red-500"}`}
                          >
                            {q.difficulty}
                          </span>
                          {q.waecYear && (
                            <span className="text-[9px] px-2 py-0.5 rounded bg-purple-500/10 text-purple-500 font-bold">
                              WAEC {q.waecYear}
                            </span>
                          )}
                        </div>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-white/60 mb-6">
              Please select the section you would like to view questions for:
            </p>
            ...
            <button
              onClick={() => setShowModal(false)}
              className="w-full mt-4 py-3 rounded-full bg-white/10 text-white font-bold hover:bg-white/20 transition-all flex items-center justify-center gap-2"
            >
              <X size={14} /> Close
            </button>
          </div>
