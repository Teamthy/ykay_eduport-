...
export default function AdminGradebookLockPage() {
  ...
  const [pendingAction, setPendingAction] = useState<{ gradebook: GradebookRow; action: "LOCK" | "REOPEN" } | null>(null);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (!generateResponse.ok) throw new Error(generateBody.error || "Unable to load class readiness.");
      setLocks(locksBody);
  ...
  async function applyLockAction() {
    ...
    try {
      const response = await fetch("/api/admin/gradebook/locks", {
        ...
        body: JSON.stringify({ gradebookId: pendingAction.gradebook.id, action: pendingAction.action }),
      });
      ...
      toast(actionError instanceof Error ? actionError.message : "Unable to update the gradebook.", "error");
    } finally {
      setBusy(false);
  ...
  async function generateReportCards() {
    ...
    try {
      ...
      toast(generateError instanceof Error ? generateError.message : "Unable to generate report cards.", "error");
    } finally {
      setBusy(false);
        ...
        <section className="bg-brand-navy px-6 pt-28 pb-14">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="mt-3 max-w-2xl font-body text-sm text-white/60">
              Lock submitted gradebooks to freeze scores, then generate draft report cards per class once every
              subject is locked.
            </p>
          </div>
        </section>
              ...
              {loading ? (
                <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-10 shadow-[var(--card-shadow)]">
                  <div className="flex items-center gap-3 text-[var(--text-secondary)]">
                    <LoaderCircle className="animate-spin text-brand-green" size={20} /> Loading gradebooks...
                  </div>
                </div>
              ) : null}
              ...
              {!loading && locks ? (
                <>
                  <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                    {[
                      { label: "Total Gradebooks", value: locks.summary.total, icon: FileText, tone: "text-brand-green" },
                      { label: "Open", value: locks.summary.open, icon: Clock, tone: "text-brand-green" },
                      { label: "Awaiting Lock", value: locks.summary.submitted, icon: Unlock, tone: "text-brand-orange" },
                      { label: "Locked", value: locks.summary.locked, icon: Lock, tone: "text-red-500" },
                    ].map((card) => (
                  ...
                  <div className="overflow-hidden rounded-[2rem] border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-[var(--card-shadow)]">
                    <div className="border-b border-[var(--border-subtle)] px-8 py-6">
                      <h2 className="font-display text-2xl text-[var(--text-primary)]">Subject Gradebooks</h2>
                      <p className="mt-1 text-xs text-[var(--text-muted)]">
                        Lock a gradebook once the teacher has submitted final scores. Reopen it only for approved corrections.
                      </p>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="border-b border-[var(--border-subtle)]">
                          <tr>
                            {["Subject", "Class", "Teacher", "Students", "Average", "Status", "Action"].map((heading) => (
                              <th key={heading} className="px-4 py-3 text-left font-display text-[10px] uppercase tracking-wider text-[var(--text-muted)]">
                                {heading}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {locks.gradebooks.map((gradebook) => (
                            <tr key={gradebook.id} className="border-b border-[var(--border-subtle)] hover:bg-[var(--surface-disabled)]">
                              <td className="px-4 py-4 font-bold text-[var(--text-primary)]">{gradebook.subjectName}</td>
                              <td className="px-4 py-4 text-xs text-[var(--text-muted)]">{gradebook.className}</td>
                              <td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{gradebook.teacherName}</td>
                              <td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{gradebook.studentCount}</td>
                              <td className="px-4 py-4 font-display text-base font-bold text-brand-green">{gradebook.classAverage}%</td>
                              <td className="px-4 py-4">
                                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${statusChip(gradebook.status)}`}>
                                  {gradebook.status === "LOCKED" ? <Lock size={10} /> : gradebook.status === "SUBMITTED" ? <Clock size={10} /> : <Unlock size={10} />}
                              ...
                              <td className="px-4 py-4">
                                {gradebook.status === "LOCKED" ? (
                                  <button
                                    onClick={() => setPendingAction({ gradebook, action: "REOPEN" })}
                                    disabled={busy}
                          ...
                          {!locks.gradebooks.length ? (
                            <tr>
                              <td colSpan={7} className="px-6 py-10 text-center text-sm text-[var(--text-muted)]">
                                No gradebooks yet. Gradebooks are created automatically when subject teachers open their score sheets.
                              </td>
                            </tr>
                          ) : null}
                        </tbody>
              ...
              {!loading && generation ? (
                <div className="overflow-hidden rounded-[2rem] border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-[var(--card-shadow)]">
                  <div className="border-b border-[var(--border-subtle)] px-8 py-6">
                    ...
                    <p className="mt-1 text-xs text-[var(--text-muted)]">
                      A class becomes ready once every subject gradebook for {generation.termLabel} is locked. Generated
                      report cards start as drafts and are released from the Report Cards page.
                    </p>
                  </div>
                  <div className="grid gap-4 p-8 md:grid-cols-2">
                    {generation.classes.map((schoolClass) => (
                      <div key={schoolClass.id} className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--card-bg-subtle)] p-6">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <h3 className="font-display text-xl text-[var(--text-primary)]">{schoolClass.displayName}</h3>
                            <p className="mt-1 text-xs text-[var(--text-muted)]">
                              {schoolClass.studentCount} students · {schoolClass.lockedGradebookCount}/{schoolClass.gradebookCount} gradebooks locked
                            </p>
                          </div>
                        ...
                        {schoolClass.subjects.length ? (
                          ...
                          <p className="mt-4 text-xs text-[var(--text-muted)]">No gradebooks for this term yet.</p>
                        )}
                        <button
                    ...
                    {!generation.classes.length ? (
                      <p className="text-sm text-[var(--text-muted)] md:col-span-2">No active classes found.</p>
                    ) : null}
                  </div>
                </div>
              ) : null}
