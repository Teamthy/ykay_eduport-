...
import {
  LayoutDashboard, Users, CreditCard, FileText, BookOpen, ClipboardCheck,
  GraduationCap, CalendarDays, Bell, Wallet, School, IdCard, Award,
} from "lucide-react";
    ...
    <>
      ...
      <main className="mx-auto max-w-[1600px] px-6 py-8">
        <div className="rounded-[2rem] bg-brand-navy p-7 text-white">
          <p className="text-xs font-bold uppercase tracking-widest text-brand-green">Cross-portal access</p>
          <h1 className="mt-2 font-display text-4xl tracking-widest">PORTAL <span className="text-brand-green">HUB</span></h1>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a href="/super-admin/schools" className="inline-flex items-center gap-2 rounded-full bg-brand-green px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white transition-all hover:scale-[1.03]">
              Schools Overview →
            </a>
            <a href="/super-admin/health" className="inline-flex items-center gap-2 rounded-full bg-blue-500 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white transition-all hover:scale-[1.03]">
              System Health →
            </a>
            <a href="/super-admin/broadcast" className="inline-flex items-center gap-2 rounded-full bg-brand-orange px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white transition-all hover:scale-[1.03]">
              Broadcast →
            </a>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          {PORTALS.map((p) => (
            <div key={p.name} className="rounded-3xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-6 shadow-[var(--card-shadow)]">
              <div className="mb-4 flex items-center gap-3">
                <span className={`grid h-11 w-11 place-items-center rounded-2xl ${p.bg} ${p.accent}`}>
                  <p.icon size={20} />
                </span>
