...
function StatusPill({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest ${ok ? "bg-brand-green/15 text-brand-green" : "bg-red-500/15 text-red-500"}`}>
        ...
        <div className="rounded-[2rem] bg-brand-navy p-7 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="font-display text-3xl tracking-widest">SYSTEM <span className="text-brand-green">HEALTH</span></h1>
            </div>
            {health && <StatusPill ok={health.status === "healthy"} label={health.status} />}
          </div>
          <p className="mt-3 text-sm text-white/65">Live infrastructure status — auto-refreshes every 15 seconds.</p>
        </div>
