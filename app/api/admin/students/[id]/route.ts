...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
    ...
    const student = await prisma.studentProfile.findFirst({
        ...
        include: {
            ...
            parentLinks: { include: { parentProfile: { select: { id: true, displayName: true, phone: true, user: { select: { email: true } } } } } },
            _count: { select: { attendanceEntries: true, reportCards: true, feeInvoices: true, gradebookEntries: true, examAttempts: true } },
        },
    });
...
const patchSchema = z.object({
    ...
    guardianEmail: z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? undefined : v), z.string().trim().toLowerCase().email().optional()),
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
    ...
    try { input = patchSchema.parse(await request.json()); } catch { return NextResponse.json({ error: "Invalid student details." }, { status: 400 }); }
    const existing = await prisma.studentProfile.findFirst({ where: { id, schoolId: user.schoolId } });
    if (!existing) return NextResponse.json({ error: "Student not found." }, { status: 404 });
    if (input.currentClassId && input.currentClassId !== existing.currentClassId) {
        const cls = await prisma.schoolClass.findFirst({ where: { id: input.currentClassId, schoolId: user.schoolId, isActive: true } });
        if (!cls) return NextResponse.json({ error: "Class not found." }, { status: 404 });
    }
    const firstName = input.firstName ?? existing.firstName;
    const lastName = input.lastName ?? existing.lastName;
    const otherNames = input.otherNames !== undefined ? (input.otherNames.trim() || null) : existing.otherNames;
    ...
    await prisma.auditLog.create({ data: { schoolId: user.schoolId, actorUserId: user.id, action: "STUDENT_UPDATED", entityType: "StudentProfile", entityId: id, ipAddress: getClientIp(request), metadata: { studentId: existing.studentId, fields: Object.keys(input) } as never } });
