...
type Exam = {
  id: string; title: string; subjectName: string; className: string;
  totalMarks: number; questionCount: number; status: string; statusLabel: string; submittedCount: number;
};
      ...
      <main className="mx-auto flex max-w-[1600px] gap-8 px-4 py-6 sm:px-6">
        ...
        <section className="min-w-0 flex-1 space-y-6">
          <div className="rounded-[2rem] bg-brand-navy p-7 text-white">
            ...
            <h1 className="mt-2 font-display text-4xl tracking-widest">TEST <span className="text-brand-green">RESULTS</span></h1>
            <p className="mt-3 max-w-2xl text-sm text-white/65">Released exam results and submission counts across your classes.</p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-4">
              ...
              <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Released</div>
            </div>
            <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-4">
              <FileCheck2 className="mb-2 text-brand-orange" size={18} />
              <div className="font-display text-2xl">{exams.reduce((s, e) => s + e.submittedCount, 0)}</div>
              <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Total submissions</div>
            </div>
            <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-4">
              <LoaderCircle className="mb-2 text-blue-500" size={18} />
              <div className="font-display text-2xl">{exams.length}</div>
              <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)]">Exams</div>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-3xl border border-[var(--border-subtle)] bg-[var(--surface-card)]">
            {loading ? (
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-[var(--surface-disabled)] text-[10px] uppercase tracking-widest text-[var(--text-muted)]">
                  <tr><th className="p-4">Exam</th><th className="p-4">Subject · Class</th><th className="p-4">Marks</th><th className="p-4">Submissions</th><th className="p-4">Status</th></tr>
                </thead>
                <tbody>
                  {exams.map((e) => (
            ...
            {!loading && !exams.length && <p className="p-10 text-center text-sm text-[var(--text-muted)]">No exams yet.</p>}
          </div>
        </section>
      </main>
    </>
