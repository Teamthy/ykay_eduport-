...
import { CURRENT_TEACHER, BEHAVIOR_RECORDS, FORM_CLASS_STUDENTS } from "@/lib/teacherData";
...
export default function BehaviorRecordsPage() {
  ...
  const [records, setRecords] = useState(BEHAVIOR_RECORDS);
  ...
  const handleAdd = () => {
    ...
    const student = FORM_CLASS_STUDENTS.find(s => s.name === newRecord.studentName);
    const record = {
    ...
    setShowModal(false);
    setNewRecord({ studentName: "", type: "Commendation", category: "", description: "", notifyParent: true });
    toast(newRecord.notifyParent ? "Record saved & parent notified" : "Record saved", "success");
  };

  const commendations = records.filter(r => r.type === "Commendation").length;
  const warnings = records.filter(r => r.type === "Warning").length;
  const notes = records.filter(r => r.type === "Note").length;

  const typeConfig = {
    Commendation: { icon: Award, color: "text-brand-green", bg: "bg-brand-green/10", border: "border-brand-green/30" },
    Warning: { icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10", border: "border-red-500/30" },
    Note: { icon: FileText, color: "text-blue-500", bg: "bg-blue-500/10", border: "border-blue-500/30" },
  };
      ...
      <main className="bg-[var(--bg-primary)] min-h-screen theme-transition">
        <section className="pt-24 pb-10 bg-brand-navy px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-white/60 text-sm">Log commendations, warnings, and notes for students in your form class.</p>
          </div>
        </section>
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-2xl bg-brand-green/10 border border-brand-green/30 p-5">
                  ...
                  <div className="text-xs uppercase tracking-widest text-brand-green mt-1">Commendations</div>
                </div>
                <div className="rounded-2xl bg-red-500/10 border border-red-500/30 p-5">
                  ...
                  <div className="text-xs uppercase tracking-widest text-red-500 mt-1">Warnings</div>
                </div>
              ...
              <div className="space-y-3">
                {records.map(r => {
                  ...
                  return (
                    <div key={r.id} className={`p-5 rounded-2xl bg-[var(--surface-card)] border-l-4 ${config.border} border-y border-r border-[var(--border-subtle)] shadow-[var(--card-shadow)]`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div>
                              ...
                              <div className="text-[11px] text-[var(--text-muted)]">{r.studentId} · {r.date}</div>
                            </div>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest ${config.bg} ${config.color}`}>
                              {r.type}
                            </span>
                          </div>
                          <div className="text-xs text-brand-orange font-bold uppercase tracking-widest mb-1">{r.category}</div>
                          <p className="text-sm text-[var(--text-secondary)] mb-3">{r.description}</p>
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowModal(false)}>
          <div className="rounded-3xl max-w-lg w-full p-8" style={{ backgroundColor: "#0C1824" }} onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Student</label>
                <select
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Type</label>
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Category</label>
                <input
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Description</label>
                <textarea
              ...
              <label className="flex items-center gap-3 p-3 rounded-xl bg-white/5 cursor-pointer hover:bg-white/10">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-white/50">Parent will receive an SMS about this record</div>
                </div>
              </label>
