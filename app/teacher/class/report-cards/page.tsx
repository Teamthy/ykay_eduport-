...
export default function ClassReportCardsPage() {
  ...
  const [selectedStudent, setSelectedStudent] = useState<typeof FORM_CLASS_STUDENTS[0] | null>(null);
  const [showRemarkModal, setShowRemarkModal] = useState(false);
  const [remark, setRemark] = useState("");

  const reportStatus = FORM_CLASS_STUDENTS.map((s, i) => ({
    ...s,
    reportStatus: i % 3 === 0 ? "Approved" : i % 3 === 1 ? "Pending Review" : "Draft",
    hasRemark: i % 2 === 0,
  }));

  const approved = reportStatus.filter(r => r.reportStatus === "Approved").length;
  const pending = reportStatus.filter(r => r.reportStatus === "Pending Review").length;
  const draft = reportStatus.filter(r => r.reportStatus === "Draft").length;

  const handleAddRemark = (student: typeof FORM_CLASS_STUDENTS[0]) => {
  ...
  return (
    <>
      ...
      <main className="bg-[var(--bg-primary)] min-h-screen theme-transition">
        <section className="pt-24 pb-10 bg-brand-navy px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-white/60 text-sm">Review and add remarks to report cards for your form class.</p>
          </div>
        </section>
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-5 rounded-2xl bg-brand-orange/10 border border-brand-orange/30">
                  ...
                  <div className="text-xs uppercase tracking-widest text-brand-orange">Pending Review</div>
                </div>
                <div className="p-5 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)]">
                  ...
                  <div className="text-xs uppercase tracking-widest text-[var(--text-muted)]">Draft</div>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)] shadow-[var(--card-shadow)] overflow-hidden">
                <div className="p-6 border-b border-[var(--border-subtle)]">
                  <h3 className="font-display text-lg text-[var(--text-primary)]">Form Class {teacher.formClass} — Report Cards</h3>
                  <p className="text-xs text-[var(--text-muted)]">First Term 2025/2026 · Add class teacher remarks before submission</p>
                </div>

                <div className="divide-y divide-[var(--border-subtle)]">
                  {reportStatus.map(s => (
                    <div key={s.id} className="p-5 flex items-center gap-4 hover:bg-[var(--surface-disabled)] transition-colors">
                      <img src={s.photoUrl} alt={s.name} className="w-12 h-12 rounded-full object-cover border-2 border-[var(--border-subtle)]" />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest ${
                            s.reportStatus === "Approved" ? "bg-brand-green/20 text-brand-green" :
                            s.reportStatus === "Pending Review" ? "bg-brand-orange/20 text-brand-orange" :
                        ...
                        <div className="text-[11px] text-[var(--text-muted)]">
                          {s.studentId} · Grade: <strong>{s.overallGrade}</strong> · Attendance: <strong>{s.attendanceRate}%</strong>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button className="p-2 rounded-lg bg-brand-green/10 text-brand-green hover:bg-brand-green hover:text-white transition-all" title="Preview">
                          <Eye size={14} />
                        </button>
                        <button className="p-2 rounded-lg bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-white transition-all" title="Submit">
                          <Send size={14} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-r from-brand-orange to-brand-orange-dark text-white flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/80 text-sm">Send all approved reports to admin for final release.</p>
                </div>
      ...
      {showRemarkModal && selectedStudent && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowRemarkModal(false)}>
          <div className="rounded-3xl max-w-lg w-full p-8" style={{ backgroundColor: "#0C1824" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowRemarkModal(false)} className="text-white/60 hover:text-white"><X size={20} /></button>
            </div>
            ...
            <div className="p-3 rounded-xl bg-white/5 mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-white/50">{selectedStudent.studentId} · Grade {selectedStudent.overallGrade}</div>
              </div>
            </div>
