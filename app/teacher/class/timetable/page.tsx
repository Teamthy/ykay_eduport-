      ...
      <main className="bg-[var(--bg-primary)] min-h-screen theme-transition">
        <section className="pt-24 pb-10 bg-brand-navy px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-white/60 text-sm">Full weekly schedule for {teacher.formClass}. Today: {today}</p>
          </div>
        </section>
        ...
        <section className="py-10 px-6">
          <div className="mx-auto max-w-7xl flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 space-y-6">
              {TIMETABLE.map(day => (
                <div key={day.day} className={`rounded-2xl overflow-hidden shadow-[var(--card-shadow)] ${day.day === today ? "border-2 border-brand-orange" : "border border-[var(--border-subtle)]"}`}>
                  <div className={`px-6 py-4 ${day.day === today ? "bg-gradient-to-r from-brand-orange to-brand-orange-dark" : "bg-brand-navy"}`}>
                      ...
                      return (
                        <div key={i} className={`flex items-center gap-4 px-4 py-3 rounded-xl ${
                          ...
                          <div className="flex-1 min-w-0">
                            <div className={`font-bold text-sm ${isBreak ? "text-brand-orange" : "text-[var(--text-primary)]"}`}>
                              {period.subject}
                            </div>
                            {period.teacher && (
