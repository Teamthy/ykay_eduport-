...
import { CURRENT_TEACHER, CLASS_ANNOUNCEMENTS } from "@/lib/teacherData";
import { useToast } from "@/components/Toast";
import { Megaphone, Plus, X, Users, User, Eye, Send, School, Clock, MessageSquare } from "lucide-react";

export default function ClassAnnouncementsPage() {
  const { toast } = useToast();
  const teacher = CURRENT_TEACHER;
  const [announcements, setAnnouncements] = useState(CLASS_ANNOUNCEMENTS);
  ...
  const handleSend = () => {
    ...
    const total = newAnnouncement.audience === "Class Only" ? 32 : newAnnouncement.audience === "Parents Only" ? 30 : 62;
    const record = {
      ...
      <main className="bg-[var(--bg-primary)] min-h-screen theme-transition">
        <section className="pt-24 pb-10 bg-brand-navy px-6">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-white/60 text-sm">Communicate with students and parents in your form class.</p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              <div className="space-y-4">
                {announcements.map(a => (
                  <div key={a.id} className="p-6 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)] shadow-[var(--card-shadow)]">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest ${
                            a.audience === "Class Only" ? "bg-brand-green/10 text-brand-green" :
                            a.audience === "Parents Only" ? "bg-brand-orange/10 text-brand-orange" :
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-[var(--text-muted)] pt-3 border-t border-[var(--border-subtle)]">
                      <span className="inline-flex items-center gap-1"><Clock size={11} /> {a.date}</span>
                      <span className="inline-flex items-center gap-1"><User size={11} /> {a.postedBy}</span>
                      <span className="inline-flex items-center gap-1 ml-auto">
                        <Eye size={11} className="text-brand-green" />
                        <span className="text-brand-green font-bold">{a.read}/{a.total}</span> read
                      </span>
                    </div>

                    {/* Read progress bar */}
                    <div className="mt-2 h-1 rounded-full bg-[var(--surface-disabled)] overflow-hidden">
                      <div className="h-full bg-brand-green" style={{ width: `${(a.read / a.total) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowModal(false)}>
          <div className="rounded-3xl max-w-lg w-full p-8" style={{ backgroundColor: "#0C1824" }} onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Audience</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      key={a.key}
                      onClick={() => setNewAnnouncement({ ...newAnnouncement, audience: a.key as any })}
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Title</label>
                <input
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-brand-orange block mb-2">Message</label>
                <textarea
