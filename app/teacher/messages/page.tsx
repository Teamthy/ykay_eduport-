...
export default function TeacherMessagesPage() {
  ...
  const [filter, setFilter] = useState<"all" | string | "unread" | "starred">("all");
  ...
  const filtered = messages.filter((m: any) => {
    ...
    return (
      matchSearch &&
      (filter === "all" ||
        (filter === "unread" ? m.unread : filter === "starred" ? m.starred : m.category === filter))
    );
  });

  const stats = {
    total: messages.length,
    unread: messages.filter((m: any) => m.unread).length,
    starred: messages.filter((m: any) => m.starred).length,
    parent: messages.filter((m: any) => m.category === "parent").length,
    student: messages.filter((m: any) => m.category === "student").length,
    admin: messages.filter((m: any) => m.category === "admin").length,
    colleague: messages.filter((m: any) => m.category === "colleague").length,
  };
  ...
  const handleSelect = (msg: any) => {
    ...
    if (msg.unread) {
      setMessages((prev) => prev.map((m: any) => (m.id === msg.id ? { ...m, unread: false } : m)));
    }
  };

  const toggleStar = (id: string) => {
    setMessages((prev) => prev.map((m: any) => (m.id === id ? { ...m, starred: !m.starred } : m)));
  };
  ...
  const handleDelete = () => {
    if (!selected) return;
    setMessages((prev) => prev.filter((m: any) => m.id !== selected.id));
  ...
  return (
    <>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all ${filter === "unread" ? "bg-brand-green/10 border-brand-green" : "bg-[var(--surface-card)] border-[var(--border-subtle)] hover:border-brand-green/30"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all ${filter === "starred" ? "bg-brand-orange/10 border-brand-orange" : "bg-[var(--surface-card)] border-[var(--border-subtle)] hover:border-brand-orange/30"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all ${filter === "parent" ? "bg-brand-orange/10 border-brand-orange" : "bg-[var(--surface-card)] border-[var(--border-subtle)] hover:border-brand-orange/30"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl border transition-all ${filter === "student" ? "bg-brand-green/10 border-brand-green" : "bg-[var(--surface-card)] border-[var(--border-subtle)] hover:border-brand-green/30"}`}
                >
              ...
              {/* Main Inbox Layout */}
              <div
                className="grid lg:grid-cols-[380px_1fr] gap-4 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-subtle)] overflow-hidden shadow-[var(--card-shadow)]"
                style={{ minHeight: "600px" }}
              >
                {/* Message List */}
                <div
                  className={`${showMobileList ? "block" : "hidden"} lg:block border-r border-[var(--border-subtle)] flex flex-col`}
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-start gap-3 pl-3">
                              <div
                                className={`w-10 h-10 rounded-full ${config.bg} ${config.color} flex items-center justify-center font-bold text-xs shrink-0`}
                              >
                                {m.from.avatar}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-2 mb-1">
                                  <div
                                    className={`text-sm truncate ${m.unread ? "font-bold text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}
                                  >
                                    {m.from.name}
                                  </div>
                                ...
                                <div
                                  className={`text-xs truncate mb-1 ${m.unread ? "font-bold text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`}
                                >
                                  {m.subject}
                                </div>
                                ...
                                <div className="flex items-center justify-between mt-1">
                                  <span
                                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-widest ${config.bg} ${config.color}`}
                                  >
                                    {config.label}
                                  </span>
                ...
                {/* Message Detail */}
                <div
                  className={`${!showMobileList ? "block" : "hidden"} lg:block flex flex-col`}
                  style={{ maxHeight: "700px" }}
                >
                  {selected ? (
                    <>
                      ...
                      <button
                        ...
                        className="lg:hidden p-4 flex items-center gap-2 text-brand-green text-sm font-bold border-b border-[var(--border-subtle)]"
                      >
                      ...
                      <div className="p-6 border-b border-[var(--border-subtle)]">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-12 h-12 rounded-full ${CATEGORY_CONFIG[selected.category as keyof typeof CATEGORY_CONFIG].bg} ${CATEGORY_CONFIG[selected.category as keyof typeof CATEGORY_CONFIG].color} flex items-center justify-center font-bold`}
                            >
                            ...
                            <div>
                              ...
                              <div className="text-xs text-[var(--text-muted)]">
                                {selected.from.role} · {selected.time}
                              </div>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <button
                              ...
                              className="p-2 rounded-lg hover:bg-[var(--surface-disabled)] transition-colors"
                            >
                              <Star
                                size={16}
                                className={
                      ...
                      <div className="flex-1 overflow-y-auto p-6">
                        ...
                        {selected.thread && selected.thread.length > 0 && (
                          <div className="mt-8 pt-6 border-t border-[var(--border-subtle)] space-y-4">
                            ...
                            {selected.thread.map((r: any) => (
                              <div
                                key={r.id}
                                className={`p-4 rounded-xl ${r.from === "Dr. Grace Okonkwo" ? "bg-brand-green/10 ml-8" : "bg-[var(--surface-disabled)] mr-8"}`}
                              >
                      ...
                      <div className="p-4 border-t border-[var(--border-subtle)] bg-[var(--surface-disabled)]">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex flex-col gap-2">
                            <button
                              className="p-2.5 rounded-xl bg-[var(--surface-card)] text-[var(--text-muted)] hover:text-brand-green transition-colors"
                              title="Attach"
                            >
                              <Paperclip size={16} />
                            </button>
      ...
      {showCompose && (
        ...
        >
          <div
            className="rounded-3xl max-w-2xl w-full p-8"
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {Object.entries(CATEGORY_CONFIG).map(([key, config]) => (
                    <button
                      key={key}
                      className={`p-3 rounded-xl bg-white/5 border border-white/10 hover:${config.bg} hover:border-brand-green/40 transition-all flex items-center justify-center gap-1 text-xs font-bold text-white`}
                    >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:outline-none focus:border-brand-green resize-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-full bg-brand-green text-white font-bold text-sm hover:bg-brand-green-dark transition-all flex items-center justify-center gap-2"
              >
